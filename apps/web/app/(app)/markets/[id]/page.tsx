...
export default function MarketDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.markets.get(id),
      api.markets.trades(id, { limit: '100' }),
    ])
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      {/* Price chart — YES price over time from trade history */}
                ...
                {trades.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-800/30 transition-colors">
                    <td className="px-5 py-2.5">
                      <div className="flex items-center gap-2">
                        <WalletAvatar address={t.wallet_address} size={22} />
                        <span className="text-gray-300">
                          {t.wallet_pseudonym && !t.wallet_pseudonym.startsWith('0x')
                            ? t.wallet_pseudonym
                            : truncateAddress(t.wallet_address)}
                        </span>
                        {t.is_whale && <span className="ml-1 text-[10px] text-yellow-400 bg-yellow-500/10 px-1.5 py-0.5 rounded">whale</span>}
                      </div>
                    </td>
